<?php
  ...
  if(isset($_REQUEST['Submit']))
  {
    ...
    if($email_user == $email){
        echo "<script>alert('Email Exist');window.open('student_reg.php','_self');</script>";
    }
    elseif($pass != $rpass)
    {
        ?>
          <script>
            ...
            window.open('student_reg.php','_self');
          </script>
          <?php
    }
    else
    {
       ...
       try{
               ...
               $mail->Subject = 'Verification Email Student Confirmation(OTP)';
...
<body class="clean-body" style="margin: 0;padding: 0;-webkit-text-size-adjust: 100%;background-color: #e2eace;line-height: inherit;"><div style="line-height: inherit;"><span style="mso-hide: all;display: none;max-height: 0px;overflow: hidden;line-height: inherit;">*|MC_PREVIEW_TEXT|*</span></div>
  ...
  <!--[if (IE)]></div><![endif]-->
                ...
                if($mail->send())
                {
                    header('Location: Sign_up_OTP_student.php');
                 }
                else
                {
